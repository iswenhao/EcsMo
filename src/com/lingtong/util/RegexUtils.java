...
public class RegexUtils {
	...
	public static String regexp(String str, String reg, int index) {
	...
	public static String[] regexp(String str, String reg) {
		Pattern pattern = Pattern.compile(reg);
		Matcher mat = pattern.matcher(str);
		String[] strs = null;
		while (mat.find()) {
			...
			for(int i=0;i<mat.groupCount();i++){
				strs[i]=mat.group(i+1);
			}
		}
		return strs;
	}
	public static void main(String[] args) {
		System.out.println(regexp("是飒飒金额：1231231元  实收金额：1231231.011元 ",".*实收金额：([0-9]+).*", 1));
		System.out.println( Pattern.matches("[0-9]{1}", "1"));
	}

	public static String subIndexStr(String value, String startStr,
			String endStr) {
		int startIndexOf = value.indexOf(startStr);
		if (startIndexOf == -1) {
			return null;
		}
		value = value.substring(startIndexOf + startStr.length(),
				value.length());
		int endIndexOf = value.indexOf(endStr);
		if (endIndexOf == -1) {
			return null;
		}
		value = value.substring(0, endIndexOf);
		return value;
